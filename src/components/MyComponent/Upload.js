    ...
    if (file.length !== 0) {
      ...
      if (change === "cover") {
        const { data } = await axios.post(
          "https://lite-friend-server.onrender.com/api/user/changeCover",
          { cover: imgUrl[0] }
        );
        setUser(data);
      } else {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={classes.uploadStory}>
      <div className={classes.uploadStory_header}>
        <div
          onClick={() => closeUpdateCoverAvatar()}
          className={classes.uploadStory_goback}
        >
          {" "}
          <ArrowBack />
          Go Back
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={classes.uploadStory}>
      ...
      <TextField
        ...
        name="studiedAt"
        defaultValue={
          userInfo.studiedAt === "undefined" ? "" : userInfo.studiedAt
        }
        type="text"
        fullWidth={true}
        inputRef={register}
