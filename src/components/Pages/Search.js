...
import { List, ListItem, ListItemText, ListItemAvatar, Button, Avatar } from '@material-ui/core/';
...
export default function Search() {
    ...
    const onSubmit = async (e) => {
        try {
            const { data } = await axios.post("https://lite-friend.herokuapp.com/api/user/search", e);
    ...
    return (
        <List dense className={classes.search}>
            ...
            {result ?
                search.map((people) => {
                    return (
                        <Link to={`/profile/${people._id}`} key={people._id} style={{ textDecoration: "none" }} >
